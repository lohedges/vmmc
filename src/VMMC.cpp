...
namespace vmmc
{
...
#ifndef ISOTROPIC
                unsigned int nPairs = model->interactionsCallback(moveParams.seed, &particles[moveParams.seed].preMovePosition[0],
                    &particles[moveParams.seed].preMoveOrientation[0], pairInteractions);
#else
                unsigned int nPairs = model->interactionsCallback(moveParams.seed,
    ...
    bool VMMC::accept()
    {
        ...
        if (isRepusive)
        {
            ...
            for (unsigned int i=0;i<nMoving;i++)
            {
                // Get a list of pair interactions.
#ifndef ISOTROPIC
                nPairs = model->interactionsCallback(moveList[i], &particles[moveList[i]].preMovePosition[0],
                    &particles[moveList[i]].preMoveOrientation[0], pairInteractions);
#else
                nPairs = model->interactionsCallback(moveList[i],
                ...
                {
#ifndef ISOTROPIC
                    energy = model->pairEnergyCallback(moveList[i],
...
#else
                    energy = model->pairEnergyCallback(moveList[i], &particles[moveList[i]].preMovePosition[0],
                        pairInteractions[j], &particles[pairInteractions[j]].preMovePosition[0]);
#endif
        ...
        for (unsigned int i=0;i<nMoving;i++)
        {
			...
			// Early exit test for large non-pairwise energies.
			if (excessEnergy > 1e6) return false;

            if (!isRepusive)
            {
#ifndef ISOTROPIC
                energy = model->energyCallback(moveList[i], &particles[moveList[i]].preMovePosition[0],
                    &particles[moveList[i]].preMoveOrientation[0]);
#else
                energy = model->energyCallback(moveList[i], &particles[moveList[i]].preMovePosition[0]);
#endif
...
#ifndef ISOTROPIC
                unsigned int nPairs = model->interactionsCallback(moveList[i], &particles[moveList[i]].preMovePosition[0],
                    &particles[moveList[i]].preMoveOrientation[0], pairInteractions);
#else
                unsigned int nPairs = model->interactionsCallback(moveList[i],
                    &particles[moveList[i]].preMovePosition[0], pairInteractions);
#endif
    ...
    void VMMC::computePostMoveParticle(unsigned int particle, int direction, Particle& postMoveParticle)
    {
        ...
        if (direction == 1)
        {
            ...
            bool isOutsideBoundary = model->boundaryCallback(particle,
                &postMoveParticle.postMovePosition[0], &postMoveParticle.postMoveOrientation[0]);
#else
            bool isOutsideBoundary = model->boundaryCallback(particle, &postMoveParticle.postMovePosition[0]);
#endif
...
#ifndef ISOTROPIC
                unsigned int nPairs = model->interactionsCallback(particle, &particles[particle].preMovePosition[0],
                    &particles[particle].preMoveOrientation[0], pairInteractions);
#else
                unsigned int nPairs = model->interactionsCallback(particle,
...
#ifndef ISOTROPIC
                        double initialEnergy = model->pairEnergyCallback(particle,
...
#else
                        double initialEnergy = model->pairEnergyCallback(particle, &particles[particle].preMovePosition[0],
                            neighbour, &particles[neighbour].preMovePosition[0]);
#endif

                        // Post-move pair energy.
#ifndef ISOTROPIC
                        double finalEnergy = model->pairEnergyCallback(particle,
                            &particles[particle].postMovePosition[0], &particles[particle].postMoveOrientation[0],
                            neighbour, &particles[neighbour].preMovePosition[0], &particles[neighbour].preMoveOrientation[0]);
#else
                        double finalEnergy = model->pairEnergyCallback(particle, &particles[particle].postMovePosition[0],
                            neighbour, &particles[neighbour].preMovePosition[0]);
#endif

                        // Pair energy following the reverse virtual move.
#ifndef ISOTROPIC
                        double reverseMoveEnergy = model->pairEnergyCallback(particle,
                            &reverseMoveParticle.postMovePosition[0], &reverseMoveParticle.postMoveOrientation[0],
                            neighbour, &particles[neighbour].preMovePosition[0], &particles[neighbour].preMoveOrientation[0]);
#else
                        double reverseMoveEnergy = model->pairEnergyCallback(particle, &reverseMoveParticle.postMovePosition[0],
                            neighbour, &particles[neighbour].preMovePosition[0]);
#endif
    ...
    void VMMC::swapMoveStatus()
    {
        ...
        for (unsigned int i=0;i<nMoving;i++)
#ifndef ISOTROPIC
            model->postMoveCallback(moveList[i], &particles[moveList[i]].preMovePosition[0], &particles[moveList[i]].preMoveOrientation[0]);
#else
            model->postMoveCallback(moveList[i], &particles[moveList[i]].preMovePosition[0]);
#endif
    }
