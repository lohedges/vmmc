...
int main(int argc, char** argv)
{
    ...
    if (dimension == 2) baseLength = std::pow((1000*M_PI)/(2.0*density), 1.0/2.0);
    else baseLength = std::pow((1000*M_PI)/(6.0*density), 1.0/3.0);
    ...
    VMMC vmmc(nParticles, dimension, coordinates, orientations, 0.15, 0.2, 0.5, 0.5, maxInteractions,
            &boxSize[0], true, &computeEnergy, &computePairEnergy, &computeInteractions, &applyPostMoveUpdates);
